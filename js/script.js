...
fetch(DATA_URL)
  .then(response => response.json()) // Convertimos la respuesta en un objeto JavaScript
  .then(data => {
    // Accedemos al array de estudiantes dentro del objeto JSON
    showData(data.students); // Llamamos a la función showData con los datos de los estudiantes
  })
  .catch(error => {
    console.error("Error al cargar los datos:", error); // Manejamos cualquier error que ocurra durante la solicitud
  });